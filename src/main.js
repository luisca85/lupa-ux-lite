...
const TIPOS = ["Problema","Oportunidad","Observación"];
const PLATAFORMAS = ["Sitio web","App móvil","App web","Otro"];
const ESTADOS = ["Planificado","En curso","Terminado"];
...
function allHeur(){ return [...HEUR_NIELSEN.map(h=>({...h,src:"Nielsen"})), ...state.customHeur.map((h,i)=>({...h,num:HEUR_NIELSEN.length+i+1,src:"Propia",custom:true}))]; }
function allSesgos(){ return [...SESGOS_BASE.map(s=>({...s,base:true})), ...state.customSesgos.map(s=>({...s,custom:true}))]; }
function heurById(id){ return allHeur().find(h=>h.id===id); }
function sesgoById(id){ return allSesgos().find(s=>s.id===id); }
function activeStudy(){ return state.estudios.find(e=>e.id===state.activeId)||null; }
...
/* ============ Vista: Hallazgos ============ */
function renderHallazgosInto(c,est){
...
/* ============ Modales / util ============ */
function openModal(title,bodyHtml,onSave,extraClass="",onClose=null){
...
function emptyInline(msg){return `<div style="grid-column:1/-1;color:var(--faint);padding:26px;text-align:center;font-size:13.5px">${esc(msg)}</div>`;}
...
function scaleImg(src,maxW,quality,maxChars){
...
function openHzLightbox(hid,start){
...
async function compositeDataUrl(d,quality){
...
async function compositeThumb(d,maxW){
...
async function mountAnnotator(container,img,opts){
...
function flowIco(){return '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="6" height="5" rx="1"/><rect x="15" y="15" width="6" height="5" rx="1"/><path d="M9 6.5h4a2 2 0 0 1 2 2v9"/></svg>';}
function activeFlow(){ return state.flujos.find(f=>f.id===state.activeFlowId)||null; }
function activeNode(){ const f=activeFlow(); return f?(f.nodes||[]).find(n=>n.id===state.activeNodeId)||null:null; }
function resolveHz(id){ return state.hallazgos.find(h=>h.id===id)||null; }
function tipoColorVar(t){ return t==="Oportunidad"?"--opor":(t==="Observación"?"--obs":"--prob"); }
async function saveActiveFlow(){ const f=activeFlow(); if(!f)return; try{ await flowStore.save(state.activeId,f); }catch(e){ console.error("saveActiveFlow",e); toast("No se pudo guardar el flujo"); } }

function renderFlujosInto(c,est){
...
function openFlow(id){ state.activeFlowId=id; const f=state.flujos.find(x=>x.id===id); state.flowMode=(f&&f.tipo==="User Journey")?"journey":"editor"; state.flowVP={x:40,y:40,k:1}; render(); }
function flowModal(f,opts){
  opts=opts||{};
  const isNew=!f; f=f||{};
...
/* ---- Editor de diagrama ---- */
function renderFlowEditor(c,flow){
...
function targetIco(){return '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="12" cy="12" r="8"/><path d="M12 2v3M12 19v3M2 12h3M19 12h3"/><circle cx="12" cy="12" r="1.6" fill="currentColor" stroke="none"/></svg>';}
...
function renderNodeDetail(c,flow,node){
...
function flagIco(){return '<svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 21V4M5 4h11l-2 3.5L16 11H5"/></svg>';}
...
function jrDefaultCarriles(){ return {touchpoint:true,emocion:true,acciones:true,expectativas:true,pain:true,oportunidades:true}; }
const JW=224;
let _jrSaveT;
function jrSave(){ clearTimeout(_jrSaveT); _jrSaveT=setTimeout(saveActiveFlow,500); }
function jrCleanupPop(){ const p=document.getElementById("emoPop"); if(p)p.remove(); if(window._jrPopClose){document.removeEventListener("pointerdown",window._jrPopClose,true);window._jrPopClose=null;} }
function emoTop(e){ return 8+(4-(e!=null?e:2))/4*50; }
function jrNormalizePaso(p){
...
function renderJourneyEditor(c,jr){
...
let JSIDE=null;
function closeJrSide(){ const r=document.getElementById("jrSideRoot"); if(r)r.remove(); JSIDE=null; }
function openJrSide(jr,idx,kind){ JSIDE={jr,idx,kind,selId:null,width:(JSIDE&&JSIDE.width)||400}; renderJrSide(); }
...
/* icons */
function plus(){return '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"><path d="M12 5v14M5 12h14"/></svg>';}
function pencil(){return '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>';}
function trash(){return '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6"/></svg>';}
function x(){return '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>';}
function searchIco(){return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><circle cx="11" cy="11" r="7"/><path d="m21 21-4-4"/></svg>';}
function camIco(){return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M4 8a2 2 0 0 1 2-2h1.6l.9-1.5h5l.9 1.5H18a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2Z"/><circle cx="12" cy="12.5" r="3.2"/></svg>';}
function chevR(){return '<svg class="chev" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m9 6 6 6-6 6"/></svg>';}
function chevL(){return '<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="m15 6-6 6 6 6"/></svg>';}
function bigIco(t){
  const p={folder:'<path d="M3 7a2 2 0 0 1 2-2h4l2 2h8a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2Z"/>',
    search:'<circle cx="11" cy="11" r="7"/><path d="m21 21-4-4"/>',
    flag:'<path d="M4 21V4M4 4h13l-2 4 2 4H4"/>',
    flow:'<circle cx="6" cy="6" r="2.4"/><circle cx="18" cy="6" r="2.4"/><circle cx="12" cy="18" r="2.4"/><path d="M8.4 6H16M6 8.4V13a3 3 0 0 0 3 3h1M18 8.4V13a3 3 0 0 1-3 3h-1"/>',
    user:'<circle cx="12" cy="8" r="3.6"/><path d="M5 20a7 7 0 0 1 14 0"/>',
    chart:'<path d="M4 20V4M4 20h16M8 20v-6M12 20V9M16 20v-9"/>',
    report:'<path d="M7 3h7l5 5v13a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1Z"/><path d="M14 3v5h5M9 13h6M9 17h6"/>'}[t];
...
function marcaActual(){ const d={wordmark:"uxuaria",nombre:"Luis Carlos Romero León",rol:"Consultor en Diseño de experiencia de usuario",sitio:"www.uxuaria.com",anio:String(new Date().getFullYear()),logo:null,headerImg:null,color:"",email:"",tel:"",servicioNombre:"MV Rescue",servicioDesc:"",servicioUrl:""}; return Object.assign(d,state.marca||{}); }
function rptSevColor(n){ return ["#8b95a5","#3aa76d","#8fbf3f","#e0a52b","#e5484d"][n]||"#8b95a5"; }
function findHz(id){ return state.hallazgos.find(h=>h.id===id)||null; }
function ensureReporte(est){
...
function saveReporte(est){ clearTimeout(_rptSaveT); _rptSaveT=setTimeout(()=>{ store.saveEstudio(est); },600); }
function linesOf(t){ return String(t||"").split("\n").map(s=>s.replace(/^[-•]\s*/,"").trim()).filter(Boolean); }
function bulletsHTML(t){ const l=linesOf(t); if(!l.length)return ""; if(l.length===1)return `<p>${esc(l[0])}</p>`; return `<ul>${l.map(x=>`<li>${esc(x)}</li>`).join("")}</ul>`; }
function paraHTML(t){ return String(t||"").split(/\n{2,}/).map(p=>p.trim()).filter(Boolean).map(p=>`<p>${esc(p).replace(/\n/g,"<br>")}</p>`).join(""); }

/* ---- Propuestas de trabajo ---- */
function defaultPropuestas(){ return [
...
function esSeedViejo(arr){ if(!Array.isArray(arr)||arr.length!==2)return false; const t=arr.map(p=>(p&&p.titulo)||""); return t[0]==="Plan mensual con fee fijo"&&t[1]==="Propuesta ad-hoc al proyecto"; }
...
function propMoney(mon,n){ const s=MONEDA_SYM[mon]||(mon?mon+" ":""); return s+" "+Math.round(Number(n||0)).toLocaleString("es-AR"); }
function propTipoLabel(t){ return t==="mensual"?"Plan fijo mensual":"Precio fijo"; }
...
function propuestaCardHTML(p,escFn,paraFn){
...
function renderReportesInto(c,est){
  if(state._flujosFor!==est.id){
...
function copyIco(){return '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="11" height="11" rx="2"/><path d="M5 15V5a2 2 0 0 1 2-2h10"/></svg>';}
...
function parseReportHash(){ const m=(location.hash||"").match(/^#reporte\/([\w-]+)/); return m?m[1]:null; }
...
async function openClientReport(estId,opts){
  opts=opts||{};
...
async function generateSharePage(est,btn){
  ...
  try{
    ...
    const boot="("+SR_boot.toString()+")("+JSON.stringify(D).replace(/<\//g,"<\\/")+");";
...
async function init(){
  ...
  if(!USE_DB && /^https?:$/.test(location.protocol)){
    let apiErr=null;
    try{
      const r=await fetch("api/health",{credentials:"same-origin",cache:"no-store",signal:AbortSignal.timeout(6000)});
      const isJson=(r.headers.get("content-type")||"").includes("application/json");
      if(r.ok&&isJson){ DB=makeHTTPAdapter("api"); USE_DB=true; DB_MODE="server"; }
      else if(isJson){ const j=await r.json().catch(()=>({})); apiErr=j.error||("Error "+r.status); }
    }catch(e){ apiErr="No se pudo contactar al servidor. Revisá la conexión."; }
    if(apiErr){
